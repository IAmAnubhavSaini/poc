...
namespace Pagination
{
    public class PageBase
    {
        ...
        public int StartIndex
        {
            get
            {
                if (this.PageCount <= this._linkPageAtATime)
                {
                    return 1;
                }
                if (this.PageCount - this.PageIndex <= _linkPageAtATime/2)
                {
                    return this.PageCount.Value - _linkPageAtATime.Value-1;
                }
                return PageIndex.Value - (_linkPageAtATime.Value/2) <= 1 ? 1 : PageIndex.Value - _linkPageAtATime.Value/2;
            }
        }
        public int EndIndex
        {
